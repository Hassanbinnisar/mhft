...
// --- 2. MIDDLEWARE & CORS ---
app.use(cors({
    origin: ["https://mhft.vercel.app", 'https://mhft-c1ra.vercel.app', "http://localhost:5173"], // Frontend live + local
    credentials: true
}));
app.use(express.json());
app.use('/uploads', express.static(uploadDir)); // Static uploads serving
...
// GET: Single Product (Zaroori for ProductDetail.jsx)
app.get('/api/products/:id', async (req, res) => {
    try {
        const product = await Product.findById(req.params.id);
        if (!product) return res.status(404).json({ message: "Product nahi mila!" });
        ...
        res.status(500).json({ message: "Server error! Product ID galat ho sakti hai." });
    }
});
...
app.post('/api/products', upload.array('images', 5), async (req, res) => {
    try {
        ...
        const newProduct = new Product({
            title: req.body.title,
            price: Number(req.body.price), // Price ko Number mein convert karna zaroori hai
            description: req.body.description,
            category: req.body.category || 'Tees',
            status: req.body.status || 'New Arrival',
            images: imagePaths, 
...
// --- 6. SERVER START ---
app.listen(PORT, () => {
    console.log(`Hassan, server is rocking on port ${PORT} 🚀`);
});